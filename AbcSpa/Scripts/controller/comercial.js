...
var appTrading = angular.module("tradingModule", ["ngCookies", "ngRoute", "ngResource", "ngDialog", "ui-notification", "chart.js",
        "angular-dialgauge", "mgcrea.ngStrap", "ng-fusioncharts", "bw.paging", "treeGrid"]);

    appTrading.controller("ComercialHomeController",
    [
        "$scope", "$route", "$location", "$http", "$cookies", '$controller', "SecurityFact", "Notification", "ngDialog", "dataProvider",
        function ($scope, $route, $location, $http, $cookies, $controller, securityFact, notification, ngDialog, dataProvider) {
            ...
            $scope.setIndexActive = function(val)
            {
                $scope.indexActive = val;
            };
         }
    ])
.controller('treeGridController', function ($scope, $timeout) {
    var tree;
