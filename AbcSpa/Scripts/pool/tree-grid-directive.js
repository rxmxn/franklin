    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          "<div class=\"table-responsive\" style=\"overflow:hidden; min-height:100px; max-height:inherit;\">\n" +
          " <table class=\"table tree-grid table-hover table-condensed table-responsive\" style=\"overflow:hidden; margin:0; max-height:inherit;\" >\n" +
          "   <thead>\n" +
          "     <tr>\n" +
          "       <th ng-repeat=\"rowAction in rowActions\">\n" +
           "         <div>{{rowAction.displayName || rowAction.field}}</div>\n" +
          "       </th>\n" +
          "       <th style=\"width:{{expandingProperty.width}}\"><a ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</a><span ng-if=\"!expandingProperty.sortable\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span><i ng-if=\"expandingProperty.sorted\" class=\"{{expandingProperty.sortingIcon}} pull-right\"></i></th>\n" +
          "       <th ng-repeat=\"col in colDefinitions\" style=\"width:{{col.width}}\" data-toggle=\"tooltip\" title=\"{{col.tooltip}}\">\n" +
          "             <a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a>\n"+
          "             <span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          ...
          "       " +
          "       <td ng-if=\"rowAction.cellTemplate\" ng-repeat=\"rowAction in rowActions\">\n" +
          "         <div ng-if=\"rowAction.cellTemplate && checklevel(rowAction.levels, row.level)\" compile=\"rowAction.cellTemplate\" cell-template-scope=\"rowAction.cellTemplateScope\"></div>\n" +
          "       </td >\n" +
          "       <td><a ng-click=\"user_clicks_branch(row.branch)\"><span ng-class=\"row.tree_icon\"\n" +
          "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              class=\"indented tree-icon\"></span></a><span class=\"indented tree-label\" ng-click=\"on_user_click(row.branch)\">\n" +
          "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +
          "       </td>\n" +
          "       <td ng-repeat=\"col in colDefinitions\">\n" +
          "         <div ng-if=\"col.cellTemplate && checklevel(col.levels, row.level)\" compile=\"col.cellTemplate\" cell-template-scope=\"col.cellTemplateScope\"></div>\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
              ...
              scope.addParam=function(row, select) {
                  scope.addParameter({ elemento: row.branch, seleccion: select});
              }
           ...
           var error, expandingProperty, expand_all_parents, expand_level, for_all_ancestors, for_each_branch, get_parent, n, on_treeData_change, select_branch, selected_branch, tree;
              ...
              scope.checklevel = function(levels, level) {
            ...
            on_treeData_change = function () {
              ...
              _ref = scope.treeData;
              _results = [];
              /*adicionado or adiel:guardar el elemento raiz al que pertenecen cada elemento de una rama*/
                _root=0;
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                root_branch = _ref[_i];
            ...
            for_each_branch(function (b, level) {
              ...
              return b.expanded = b.level < expand_level && (b.MostrarLista === undefined || b.MostrarLista === true);
            });
