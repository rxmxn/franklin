...
// Modified by Ramon to use the metro ui wizard 2 template

angular.module("step.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("step.html",
      "<div class=\"step\" ng-class=\"{next: !completed && !selected, active: selected, prev: completed && !selected}\" style=\"height: inherit;\">\n" +
      "    <div class=\"step-content\" ng-show=\"selected\" ng-transclude style=\"height: inherit;\">\n" +
      "    </div>\n" +
      "</div>");
}]);
...
angular.module("wizard.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("wizard.html",
      "<div class=\"wizard2\">\n" +
      "    <div id=\"steps\" ng-transclude style=\"height: 99%;\"></div>\n" +
      "    <div class=\"action-bar next\" >\n" +
      "        <button wz-previous class=\"button cycle-button medium-button wiz-btn-prev place-left\" ng-show=\"currentStepNumber() > 1\">&lt;</button>\n" +
      "        <button wz-finish class=\"button cycle-button medium-button wiz-btn-finish place-right\" ng-show=\"currentStepNumber() === (getEnabledSteps().length)\"><i class=\"fa fa-check\"></i></button>\n" +
      "        <button wz-next class=\"button cycle-button medium-button wiz-btn-next place-right\" ng-show=\"currentStepNumber() < (getEnabledSteps().length)\">&gt;</button>\n" +
      "    </div>\n" +
      "</div>\n" +
      "");
}]);
    ...
    return {
        ...
        controller: ['$scope', '$element', '$log', 'WizardHandler', '$q', function ($scope, $element, $log, WizardHandler, $q) {
            ...
            this.next = function (callback) {
                //console.log("onclick only in next");
                if ($scope.onClicknext) {
                    $scope.onClicknext();
                }
            ...
            // Added by Ramon
            var calcWidth = function (stepIndex) {
                var _steps = $element.context.children.steps.children;
                var _width = $element.innerWidth() - ((_steps.length - 1) * 24 + (_steps.length));

                var action_bar = $element.find('.action-bar');
                var left = stepIndex * 25;

                var index = stepIdx($scope.selectedStep);

                var curr_frame = $element.find('.step.active');
                if (curr_frame.length === 0) {
                    curr_frame = $element.find('.step.ng-isolate-scope');
                    curr_frame.css({
                        width: _width
                    });
                    ...
                    var upcoming_frame = curr_frame[0];
                    var uframe = index < stepIndex ? upcoming_frame.nextElementSibling : upcoming_frame.previousElementSibling;

                    uframe.style.width = _width + 'px';
