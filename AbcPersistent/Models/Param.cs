...
namespace AbcPersistent.Models
{
	...
	public class Param
	{
		...
		public Param()
		{
			...
			Active = true;
			Formula = "NA";
			TipoFormula = TipoDeFormula.Otra;
		...
		public enum TipoDeFormula { MediaGeometrica, MediaAritmetica, PromedioPonderado, Otra };
		...
		public TipoDeFormula TipoFormula { get; set; }
			...
			if (!sheet)
			{
				return new
				{
					...
					? new
					{
                        ...
                        AnnalistKeys=UnidadAnalitica.AnnalistKeys?.Where(a=>a.Active).Select(a=>a.ToJson())
					} : null,
					Rama = (Rama != null)
					? new
					{
					...
					QcObj = QcObj.ToJson(),
                    AnnalistKey = AnnalistKey?.ToJson(),
					...
					Signatarios = Annalists.Where(a => a.Active && a.RecAdqs
						...
						&& ro.Params.Any(p => p.Id.Equals(Id)))) /*&& a.AnnalistKey != null*/).Select(a => a.Key/*a.AnnalistKey.Clave*/),
					Eidas = Annalists.Where(a => a.Active && a.RecAdqs
						.Any(ra => ra.NivelAdquirido.Equals(RecAdq.AcquiredLevel.Eidas)
						&& ra.RecOtorgs.Any(ro => !ro.Enterprise.Tipo
						&& ro.Params.Any(p => p.Id.Equals(Id)))) /*&& a.AnnalistKey!=null*/).ToList().Select(a => a.Key/*a.AnnalistKey.Clave*/),
					Annalists = Annalists.Select(a => new
					{
						a.Id,
						Name = a.ToString(),
						a.Key/*Key=a.AnnalistKey?.ToJson()*/
					}),
					Metodo = (Metodo != null)
					? new
					{
     ...
     //               Matrix = Matrix != null ? new
		...
		public dynamic ToMiniJson(ParamRoute paramRoute, int? DReporte=null, int groupId = 0)
		{
			return new
			{
				...
				Annalists = Annalists?/*.Where(a=>a.AnnalistKey!=null).ToList()*/.Select(a => a.Key/*a.AnnalistKey.Clave*/),
